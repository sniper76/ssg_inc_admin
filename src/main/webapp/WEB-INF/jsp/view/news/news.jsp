						...
						<ul>
							<li class="mid_in">
								<div class="mid_in_list">
									<div class="box_tit">
										<span class="tit">메인 - 소식 관리</span>
										<button type="button" id="saveBtn">저장하기</button>
									</div>
									<div class="box_con">
										<form action="<c:url value='/news/register.do' />" id="form1" name="form1" enctype="multipart/form-data" method="post">
...
{{/if}}
												...
												<tr>
													...
													<td class="cell" colspan="3">
														<input name="linkUrl" type="text" class="inp_txt02" value="{{:linkUrl}}" placeholder="이동할 url을 입력해주세요" />
													</td>
												</tr>
											</table>
										</div>
...
<script>
	$(document).ready(function(){
		$("#saveBtn").click(function(){
			...
			document.form1.action = "<c:url value='/news/register.do' />";
	...
	function delDiv(obj, key) {
		...
		if(key == '') {
		    ...
		    $.ajax({
		        type: "POST",
		        url: "<c:url value='/news/deleteAjax.do' />",
		        dataType: "json",
	...
	function getList() {
	    $.ajax({
	        type: "POST",
	        url: "<c:url value='/news/listAjax.do' />",
	        dataType: "json",
			contentType:"application/json;charset=UTF-8",
